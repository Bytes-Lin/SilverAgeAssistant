...
模拟器和真机都需要能路由到开发者自行配置的服务地址，并确保监听接口与防火墙规则允许访问。真实地址只写入未入库的 `AndroidAgent/dev.properties`。
...
```properties
modelBaseUrl=https://model-provider.example.invalid
...
```bash
curl https://model-provider.example.invalid/v1/chat/completions \
