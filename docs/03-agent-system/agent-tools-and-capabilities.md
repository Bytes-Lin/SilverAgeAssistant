...
ReAct 正常推进不设固定总步数上限。执行器根据目标包、窗口标题、当前无障碍节点的文字、
描述、类型、位置和可交互属性生成页面指纹，并根据动作类型、目标语义、输入内容或坐标区域
生成步骤指纹。同一页面的同一步骤允许执行 5 次，第 6 次仍准备重复时，本次 GuiRun 才失败；
进入新页面或采用不同动作不会增加原步骤计数；之后回到相同页面再次重复相同步骤会继续累计，
从而识别页面之间来回循环。老人提供新的语音信息会清空重复计数。该失败仍只算一次完整运行失败。
