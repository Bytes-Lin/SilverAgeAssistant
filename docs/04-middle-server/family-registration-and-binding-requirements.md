...
Android 当前只做 6 至 20 位数字的手机号格式预检。中台必须统一完成号码规范化和有效性验证；首版若默认中国大陆区号，应在 OpenAPI 中明确默认规则。数据库比较、唯一索引和限流均使用规范化后的号码，响应与日志只返回掩码。
...
家属手机号必须先通过中台认可的身份验证流程。短信 OTP、开发账号或其他验证机制可由中台决定，但不得仅凭客户端提交手机号就标记为已验证。短信验证码不得写日志或持久化到业务事件表。
...
```json
{
  ...
  "verification_token": "short-lived-verification-result",
...
| 401 | `FAMILY_MOBILE_NOT_VERIFIED` | 家属端：“请先完成手机号验证。” |
